...
# 对 $RUNS_DIR（默认 runs/rcr）下所有已完成 run 计算
...
# 结果追加写入 $RUNS_DIR/eval_hard_results.txt
...
# 300 轮协议目录：
#   sbatch -w control06 --dependency=afterok:<训练数组ID>_% \
...
OUT=${RUNS_DIR:-runs/rcr}/eval_hard_results.txt
mkdir -p "$(dirname "$OUT")"
...
for d in ${RUNS_DIR:-runs/rcr}/*/weights/best.pt; do
